...
public class AudioPickAdapter extends BaseAdapter<AudioFile, AudioPickAdapter.AudioPickViewHolder> {
    ...
    public void onBindViewHolder(final AudioPickViewHolder holder, final int position) {
        ...
        if (holder.mTvTitle.getMeasuredWidth() >
                FilePickerUtil.getScreenWidth(mContext) - FilePickerUtil.dip2px(mContext, 10 + 32 + 10 + 48 + 10 * 2)) {
            holder.mTvTitle.setLines(2);
        } else {
            holder.mTvTitle.setLines(1);
        }

        holder.mTvDuration.setText(FilePickerUtil.getDurationString(file.getDuration()));
        if (file.isSelected()) {
            ...
            public void onClick(View v) {
                try{
                    ...
                    if (FilePickerUtil.detectIntent(mContext, intent)) {
                        ...
                        FPToastUtil.getInstance(mContext).showToast(mContext.getString(R.string.vw_no_audio_play_app));
                    }
                }catch (Exception e){
