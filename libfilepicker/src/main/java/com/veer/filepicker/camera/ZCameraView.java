...
import com.veer.filepicker.camera.state.CameraMachine;
 ...
 * <li>Description: </li>
 */
public class ZCameraView extends FrameLayout implements CameraInterface.CameraOpenOverCallback, SurfaceHolder
        .Callback, CameraView {
//    private static final String TAG = "JCameraView";
    ...
    //回调监听
    private ZCameraListener zCameraLisenter;
    ...
    public ZCameraView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        duration = a.getInteger(R.styleable.ZCameraView_duration_max, 16 * 1000);       //没设置默认为16s
        a.recycle();
        initData();
        initView();
    }
    ...
    private void initView() {
        ...
        mClose.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                if(mCaptureLayout.view_confirm.getVisibility()==View.VISIBLE){
...
//                    Log.i("CJT", "result = " + (result - firstTouchLength));
    ...
    public void setZCameraLisenter(ZCameraListener jCameraLisenter) {
        this.zCameraLisenter = jCameraLisenter;
    }
    ...
    public void confirmState(int type) {
        switch (type) {
            case TYPE_VIDEO:
                ...
                mVideoView.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
                machine.start(mVideoView.getHolder(), screenProp);
                if (zCameraLisenter != null) {
                    zCameraLisenter.recordSuccess(videoUrl, firstFrame);
                }
                break;
            case TYPE_PICTURE:
                mPhoto.setVisibility(INVISIBLE);
                if (zCameraLisenter != null) {
                    zCameraLisenter.captureSuccess(captureBitmap);
                }
                break;
            case TYPE_SHORT:
                break;
    ...
    public void setBottomConfirmText(String textString,String btnString){
        if(mCaptureLayout!=null) mCaptureLayout.setBottomConfirmText(textString,btnString);
    }
    @Override
